      ...
      await client.query(
        `
          ...
          SET amount_paid = $1::numeric,
              ...
              order_status = CASE
                WHEN COALESCE(order_status, 'pending') = 'pending' THEN 'processing'
                ELSE order_status
              END,
              status_changed_at = CASE
                WHEN COALESCE(order_status, 'pending') = 'pending' THEN CURRENT_TIMESTAMP
...
async function completePayment(client, payment, result) {
  ...
  await client.query(
    `
      ...
      WHERE id = $8::integer
    `,
    [receivedAmount.toFixed(2), phone, receipt, resultCode, resultDesc, JSON.stringify(result), matched ? 'matched' : 'mismatch', payment.id]
  );

  let settledOrder = null;
  if (matched && payment.order_id) {
  ...
  return { matched, receivedAmount, expectedAmount, receipt, phone, resultCode, resultDesc, settledOrder };
}

function emitPaymentEvent(payment, outcome) {
  try {
    ...
    const payload = {
      ...
      reconciliation_status: outcome.matched ? 'matched' : outcome.status === 'completed' ? 'mismatch' : 'manual_review',
    ...
    if (outcome.status === 'completed') ws.broadcastPaymentCompleted(payload);
    else if (ws.broadcastPaymentFailed) ws.broadcastPaymentFailed(payload);
  } catch (error) {
...
async function queryPaymentStatus(req, res) {
  ...
  try {
    let payment = await getPayment(client, checkoutRequestId, false);
    if (!payment) return handleError(res, 404, 'Payment not found');

    const status = String(payment.status || '').toLowerCase();
    const ageMs = Date.now() - new Date(payment.created_at).getTime();

    if (status !== 'completed' && status !== 'manually_resolved' && ageMs >= 4000) {
      try {
        const darajaResult = await darajaStatus(checkoutRequestId);
        if (darajaResult.ResultCode !== undefined || darajaResult.ResultDesc) {
          await client.query('BEGIN');
          payment = (await getPayment(client, checkoutRequestId, true)) || payment;

          let outcome;
          if (Number(darajaResult.ResultCode) === 0) {
            outcome = { ...(await completePayment(client, payment, {
              ResultCode: 0,
            ...
            await client.query(
              `UPDATE payments
               SET status = $1,
                   ...
                   updated_at = CURRENT_TIMESTAMP
               WHERE id = $4::integer`,
              [next, Number(darajaResult.ResultCode), String(darajaResult.ResultDesc || ''), payment.id]
            );
            outcome = {
              status: next,
              matched: false,
              ...
              resultCode: Number(darajaResult.ResultCode),
              resultDesc: String(darajaResult.ResultDesc || ''),
              settledOrder: null,
            };
          }
          ...
          emitPaymentEvent(payment, outcome);
        }
      } catch (fallbackError) {
    ...
    payment = await getPayment(client, checkoutRequestId, false) || payment;
...
async function mpesaCallback(req, res) {
  ...
  try {
    ...
    await client.query('BEGIN');
    const payment = await getPayment(client, checkoutRequestId, true);
    if (!payment) {
      await client.query('COMMIT');
    ...
    if (resultCode === 0) {
      const outcome = { ...(await completePayment(client, payment, result)), status: 'completed' };
      await client.query('COMMIT');
      emitPaymentEvent(payment, outcome);
    ...
    await client.query(
      `UPDATE payments
       SET status = $1,
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $5::integer`,
      [next, resultCode, String(result.ResultDesc || ''), JSON.stringify(result), payment.id]
    );
    await client.query('COMMIT');
    ...
    return handleSuccess(res, 200, 'Payment callback processed', { checkoutRequestId, status: next, resultDesc: String(result.ResultDesc || '') });
  } catch (error) {
    try { await client.query('ROLLBACK'); } catch (_) {}
